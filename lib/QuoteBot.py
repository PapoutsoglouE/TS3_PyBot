			...
			if m[:6].lstrip().lower() == self.trigger: 
				with open(self.filename, "r+") as f:
					...
					if m.strip().lower() == self.trigger:
						...
						add_match = re.compile(self.trigger + r"\s+add\s+(.*)", flags=re.IGNORECASE)
						...
						print("found remove")
						rem_match = re.compile(self.trigger + r"\s+remove\s+(.*)", flags=re.IGNORECASE)
						...
						if str(rem_id) in quote_IDs:
							...
							f.truncate()
							f.seek(0)
							return ("[b]QuoteBot[/b]: Removed quote [" + rem_id + "]: " + q)
					
					elif parts[1] ==  "count":
						return "[b]QuoteBot[/b]: " + str(len(quote_IDs)) + " quotes found."
